...
public class ClusterSumFieldGroupingStormTopology {
    ...
    public static class DataSourceSpout extends BaseRichSpout {
        ...
        public void declareOutputFields(OutputFieldsDeclarer declarer) {
            declarer.declare(new Fields("flag", "number"));
        }
    }
    ...
    public static void main(String[] args) {
        ...
        //这而的分组策略要按：实际场景来设定
        //比如此例：按奇数、偶数进行字段分组，设置成3个并行度，会导致有一个线程空闲，没有事干，造成资源的浪费
        //假设将并行度设置成1，会有怎样的结果：
        //一个线程处理，并不会导致数据的丢失
        builder.setBolt("SumBolt", new SumBolt(), 2).fieldsGrouping("DataSourceSpout", new Fields("flag"));
